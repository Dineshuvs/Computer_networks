...
#	endif
	...
	while( 1 )
	{
		...
		printf("connection[i].sock %d",connections[0].sock );
        for( size_t i = 0; i < connections.size(); ++i ) 
		{
			printf("connectionsize %d",connections.size );
			printf("connection[i].sock %d",connections[i].sock );
			if(connections[i].sock >= maxfds)
				maxfds = connections[i].sock + 1;
		...
		int ret = select ( maxfds, &readfds, &writefds , 0,0);
...
#			endif
		...
		for (size_t i=0; i< connections.size();++i)
		{
		  ...
		  continue;
			...
			bool socketprocess = true;
			if( FD_ISSET( connections[i].sock, &writefds ) )
			{
				socketprocess = process_client_send(connections[i] );
				if(socketprocess == false) 
				{
			...
			else if( FD_ISSET( connections[i].sock, &readfds ) )
			{
				socketprocess = process_client_recv( connections[i] );
				if(socketprocess == false) {
					close(connections[i].sock);
					FD_CLR(connections[i].sock, &readfds);		
					connections[i].sock = -1;
		...
		connections.erase(
			std::remove_if(connections.begin(), connections.end(), 
				&is_invalid_connection),connections.end());
